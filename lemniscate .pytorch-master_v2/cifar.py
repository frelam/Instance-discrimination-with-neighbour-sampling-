...
parser.add_argument('--nce-k', default=512, type=int,
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets, indexes) in enumerate(trainloader):
        ...
        features = net(inputs)
        outputs = lemniscate(features, indexes, epoch)
        loss = criterion(outputs, indexes)

        #indexes_memory = lemniscate.get_memory(indexes)
        #loss_norm = torch.norm(features - indexes_memory)/128

        #loss = torch.add(loss, loss_norm)
        loss.backward()
        optimizer.step()
...
for epoch in range(start_epoch, start_epoch+200):
    ...
    if not os.path.isdir('acc_curve'):
        os.mkdir('acc_curve')
    accwrite = [acc]
    if not os.path.isdir('acc_curve'):
        os.mkdir('acc_curve')
    #out = open('./acc_curve/resnet_50_k=10_m=1000_cifar10_our_work.csv', 'a', newline='')
    out = open('./acc_curve/resnet_50_k=512_m=2048_cifar100_our_work1.csv', 'a', newline='')
    csv_write = csv.writer(out, dialect='excel')
    csv_write.writerow(accwrite)
    if acc > best_acc:
        print('Saving..')
        state = {
            'net': net.state_dict(),
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/ckpt_k=512.t7')
        best_acc = acc
