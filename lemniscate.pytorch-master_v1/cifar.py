...
parser.add_argument('--resume', '-r', default='ckpt_cifar100_20.t7', type=str, help='resume from checkpoint')
...
if args.test_only:
    acc = kNN(0, net, lemniscate, trainloader, testloader, 1000, args.nce_t, 1)
    sys.exit(0)
...
for epoch in range(start_epoch, start_epoch+200):
    ...
    if not os.path.isdir('acc_curve'):
        os.mkdir('acc_curve')
    out = open('./acc_curve/resnet_50_k=20_cifar100.csv', 'a', newline='')
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/ckpt_cifar100_20.t7')
        best_acc = acc
