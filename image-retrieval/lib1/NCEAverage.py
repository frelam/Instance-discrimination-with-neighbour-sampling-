...
class NCEFunction(Function):
    ...
    def forward(self, x, y, memory, idx, params):
        ...
        weight = torch.index_select(memory, 0, idx.view(-1))  # weight 即identities feature，公式里的v
        ...
        out.div_(T).exp_() # batchSize * self.K+1   # exp(vfi/t)
...
class NCEAverage(nn.Module):
    ...
    def __init__(self, inputSize, outputSize, K, T=0.07, momentum=0.5, Z=None):
        ...
        self.register_buffer('memory', torch.rand(outputSize, inputSize).mul_(2*stdv).add_(-stdv))  #t-i
 
    def forward(self, x, y):
        batchSize = x.size(0)
        idx = self.multinomial.draw(batchSize * (self.K+1)).view(batchSize, -1)  # 得到采样样本
