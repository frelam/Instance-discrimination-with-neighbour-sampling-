...
class NCEFunction(Function):
    @staticmethod
    def forward(self, x, y, memory, idx, idx2, params):
        K = int(params[0].item())
        T = params[1].item()
        Z = params[2].item()
        ...
        weight.resize_(batchSize, 8192 + 1, inputSize)
        ...
        out_for_sort = out.narrow(1, 1, 8192)
        ...
        #indices_cpu = [4096 + i - int(K) for i in range(int(K))]
        #indices_cpu = [i for i in range(int(K))]
        #indices_cpu = [2^i for i in range(int(K))]
        indices_cpu = idx2
        ...
        indexes_sort_temp = torch.cuda.LongTensor(batchSize, K)
        for i in range(batchSize):
            indexes_sort_temp[i] = torch.index_select(indexes_sort[i], 0, indices[i])

        #idx_sorted = torch.cat((torch.cuda.LongTensor(batchSize, 1).zero_(),torch.index_select(indexes_sort, 1, indices)),1)
        idx_sorted = torch.cat((torch.cuda.LongTensor(batchSize, 1).zero_(), indexes_sort_temp), 1)
        ...
        #indices3_cpu = [0]+[i for i in range(20, int(K + 1))]
        #indices3 = torch.cuda.LongTensor(indices3_cpu)
        if Z < 0:
            #params[2] = torch.index_select(out, 1, indices3).mean() * outputSize  # Z = n/m*sum(exp(vfi/t))  n=outputSize
...
class NCEAverage(nn.Module):
    ...
    def __init__(self, inputSize, outputSize, K, T=0.07, momentum=0.5, Z=None):
        ...
        self.unigrams2 = torch.ones(4096)
        #self.unigrams3 = torch.ones(1500)
        self.multinomial = AliasMethod(self.unigrams)
        self.multinomial2 = AliasMethod(self.unigrams2)
        #self.multinomial3 = AliasMethod(self.unigrams3)

        # 采样方法
        self.multinomial.cuda()
        self.multinomial2.cuda()
        #self.multinomial3.cuda()
    ...
    def forward(self, x, y):
        batchSize = x.size(0)
        idx = self.multinomial.draw(batchSize * (8192+1)).view(batchSize, -1)  # 得到采样样本 +1+1+1!!!!!!
        '''
        if epoch < 130:
            idx2 = self.multinomial2.draw(batchSize * self.K).view(batchSize, -1)#+3500
        else:
            idx2 = self.multinomial3.draw(batchSize * self.K).view(batchSize, -1)
        '''
        idx2 = self.multinomial2.draw(batchSize * self.K).view(batchSize, -1)
        #idx2 = self.multinomial2.draw(self.K) + 6
        #idx1 = self.multinomial2.draw(batchSize * int(6*self.K/8)).view(batchSize, -1)
        #idx3 = self.multinomial3.draw(batchSize * int(2*self.K/8)).view(batchSize, -1)+1500
        #idx2 = torch.cat((idx1,idx3), 1)
        out = NCEFunction.apply(x, y, self.memory, idx, idx2, self.params)
        return out
